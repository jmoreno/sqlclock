...
char* sqlcode_get_message(char *code) {
  ...
  {
    if (strncmp(code, sqlcodes[i],5)==0)
    {
      change_time = 1;
      return sqldescriptions[i];
    }
  }
  
  change_time = 0;
  return "This time doesn't seem to be a SQLCODE or at least, none with a short description. Wait the next minute. ;)";
